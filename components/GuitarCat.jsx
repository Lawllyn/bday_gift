...
export default function GuitarCat({ step, isFinished }) {
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      if (keysPressed.current['arrowleft'] || keysPressed.current['a']) {
        posXRef.current = Math.max(0, posXRef.current - stepSpeed);
        setIsFlipped(false);
  ...
  // 6. Key Listeners (Langsung sembunyikan hint saat user interaksi)
  useEffect(() => {
    if (isMobile) return;

    const handleKeyDown = (e) => {
      // Langsung hilangkan hint saat tombol apa saja ditekan
      setShowHint(false);
  ...
  return (
    <>
      <style>{`
        ...
        .cat-jumping {
          animation: catJump 0.45s cubic-bezier(0.28, 0.84, 0.42, 1);
        }
      `}</style>
      ...
      >
        ...
        {/* Gambar Kucing */}
        <div className={isJumping ? 'cat-jumping' : ''}>
          <img 
            src="foto/cat.png"
